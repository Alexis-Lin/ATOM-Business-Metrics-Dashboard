...
| [`frontend-api-contract.ts`](frontend-api-contract.ts) | 前端 ↔ 后端 | **API 接口契约**：TypeScript 类型 + 端点一览。枚举与后端字段严格对齐（四层活跃 a1–a4、`workout_length_type`、`app_category`、用户三档分层等） |
| [`backend-warehouse.sql`](backend-warehouse.sql) | 后端/数据 | **数仓建表 + 指标推导 SQL**（一个文件两部分）：Part 1 全部 DDL（维表、事实表、双基座 `agg_user_daily` / `agg_membership_daily`、`global_metrics_daily`）；Part 2 各模块指标推导示例（有效训练判定、ENG01 矩阵、ENG08/08R、ENG14 三档分层、RET01 周 cohort、TRN08/19、GLOBAL 汇总） |
...
- 用户分层三档（D38，近 28 天）：重度 ≥10 次 / 常规 4–9 / 轻度 1–3；0 次不入档。

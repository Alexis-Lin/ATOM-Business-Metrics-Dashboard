...
- 用户分层（D43 四档，近 28 天，基数 = 有开机的账号）：重度 ≥10 次 / 常规 4–9 / 轻度 1–3 / 零训练（0 次但有开机）；完全不开机不入档
- 口径：UV = 按 user_id/device_id 去重（默认）；PV = 次数累计；业务日 = 美国太平洋时间 PT
- North Star（★ 五角星仅标北极星，D42）：NSM-1 = `ENG08` 周训练活跃 ★；NSM-2 = `SUB02` 活跃付费 Pro ★

## 状态

口径十三轮对齐完成（D1–D44，指标已瘦身：7 项退役 / 2 项降二期 / 3 项转运营名单；SUB/DEV Tab 一期置灰）。余项：R16 兑换有效期窗口、R17 Pro 各区价格表、R19 桌面参与阈值、R20 完课阈值（占位 70%）。下一步：后端按 `dev/backend-warehouse.sql` 建仓接真实数据，前端按 `dev/README.md` 三步把原型接到 API——数据准了再上。
